...
if (!pfxPath || !passphrase || !endpoint || !xmlPath) {
  console.error(`
Uso:
  tsx scripts/anular-facturas-xml.ts <pfxPath> <passphrase> <endpoint> <xmlPath>

Ejemplo:
  tsx scripts/anular-facturas-xml.ts ./certs/cert.pfx 1234 https://aeat-pre... ./anulacion.xml
  `);
  process.exit(1);
}
...
(async () => {
  try {
    ...
    console.error('[❌ Error al anular]', err.message || err);
    process.exit(1);
  }
})();
